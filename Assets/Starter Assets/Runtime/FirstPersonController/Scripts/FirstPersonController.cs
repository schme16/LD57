...
namespace StarterAssets {
	...
	public class FirstPersonController : MonoBehaviour {
		...
		public float timeBetweenSprintingFootsteps = 0.3f;
		...
		private void Move() {
			...
			if (Grounded && (_input.move.x != 0 || _input.move.y != 0)) {
				HandleFootsteps(currentHorizontalSpeed);
			}
		...
		private void HandleFootsteps(float speed) {
			...
			if (Time.time > nextStepTime && speed > walkSpeedThreshold) {
				OnPlayerWalk.Invoke();
				nextStepTime = Time.time + (_input.sprint ? timeBetweenSprintingFootsteps : timeBetweenFootsteps);
			}
		}
	}
}
